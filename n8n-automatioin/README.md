...
4. Các node theo **từng job**: sau **`6b`**, **`6d`** — nếu **`youtube_posted`** thì **bỏ qua** chuỗi **`7`–`12`** và vào **`6c`** (chỉ FB); nếu chưa posted thì **`7` → `8` → `8c` → `8d` → `9`**. **`8d Gộp video và thumbnail`** ghép lại **`binary.data`** (video) và **`thumbnail`** — sau **`8c`**, Read File thường **chỉ giữ** binary vừa đọc nên **`9`** dễ báo *no binary field `data`*. **`9b`** / **`9c`**: upload YouTube lỗi (`continueOnFail` trên **`9`**) → **vẫn sang `12b`** để kiểm tra Facebook; có **`id`** → **`10` → … → `12a` → `12b`**. **`12b`** không bắt buộc có YouTube `id` để đọc video cho FB. **`12a`** trước **`12b`**; **`6e`** — **`facebook_posted`** → **`19`**, không → **`13`–`18`**. (**Không dùng `Split In Batches`**.)
