...
App dùng **client_id**, **client_secret**, **refresh_token** với scope **`https://www.googleapis.com/auth/youtube.upload`**.

1. **Google Cloud Console**: **https://console.cloud.google.com/** → project → **APIs & Services** → bật **YouTube Data API v3**.
2. **Credentials** → **OAuth client ID** — nên dùng kiểu **Web application** (giống n8n).
3. Trong OAuth client, mục **Authorized redirect URIs** thêm: **`http://127.0.0.1:8742/`** (cổng cố định cho SubSync; khác `localhost:5678` của n8n nếu bạn dùng song song).
4. Trong app: tab **Publish profiles** → nhập **Client ID** + **Client secret** → bấm **«Lấy refresh token (mở trình duyệt)»** → đăng nhập Google → token được điền vào ô refresh (nhớ **Save**).
5. Hoặc CLI (cùng luồng): từ thư mục project, `python scripts/youtube_oauth_refresh_token.py --client-id "..." --client-secret "..."` — in refresh token ra stdout.
6. Cách khác: **OAuth Playground** hoặc tài liệu **https://developers.google.com/youtube/v3/guides/auth/server-side-web-apps**
