...
Video gốc (Trung/Anh/Nhật...)
  ...
  ③ Burn Subtitle → ghép phụ đề vào video
  ④ Separate Voice→ tách giọng người / nhạc nền
  ⑤ Add Voice TTS → thêm giọng đọc tiếng Việt
```
...
│       ├── step3_burn.py          ← Burn subtitle (ffmpeg)
│       ├── step4_separate.py      ← Tách giọng (Demucs)
│       └── step5_tts.py           ← Text-to-Speech
│
└── ui/
    ├── main_window.py             ← Cửa sổ chính PyQt6
    └── widgets/
...
<base_dir>/<tên_file>_<YYYYMMDD_HHMMSS>/
  ...
  └── step5_output.mp4          ← video cuối (sub + TTS)
```
